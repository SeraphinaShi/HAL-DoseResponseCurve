...
plot_performences_cv_ug_alla_noBT <- function(df, save_plot=NA, return_plot = "all"){
...
plot_perforences_alllambda_noBT <- function(df, u_g_scaler=NA, u_l_scalers=NA, save_plot=NA, max_bias_sd=NA){
  ...
  for (i in 1:length(eval_points)) {
    ...
    p_est_avg = ggplot(df_a) +  
      ...
      # geom_ribbon(aes(x = lambda_scaler, ymin=ci_lwr_bt, ymax=ci_upr_bt,  color='Bootstrap', fill = 'Bootstrap'),  alpha=0.5) +
...
plot_performances_uHAL_gam_poly_noBT <- function(df, save_plot=NA){
  ...
  df <- df[df$method %in% c("0_HAL", "U_SOadapt_HAL", "GAM", "Poly"), ]
...
run_simu_rep_summary <- function(results_list){
  ...
  for (method in c("CV", "U_G")){
    ...
    result_all <- merge(as.data.frame(psi0_pnt), result_all, by=c("a"))
    # result_all_5 = result_all[result_all$a == 5,]
...
plot_perforences_alllambda_noBT_extra <- function(df, u_g_scaler=NA, u_l_scalers=NA, save_plot=NA, max_bias_sd=NA){
...
plot_perforences_alllambda_noBT_extra_lambda <- function(df, u_g_lambda=NA, cv_lambda=NA, u_l_scalers=NA, save_plot=NA, max_bias_sd=NA){
...
plot_perforences_alllambda_noBT_extra_lambda_a <- function(df, a=1, u_g_lambda=NA, cv_lambda=NA, save_plot=NA, max_bias_sd=NA){
  ...
  p <- grid.arrange(p_est_avg, 
                    p_bias, p_se, 
                    p_bias_se, p_cr, 
                    legend, legend_undersmoothing, 
                    layout_matrix = rbind(c(1,1,5,5, 6,7),
                                          #c(1,1,NA,7),
                                          c(2,2,3,3,4,4))
  )
                    # top = textGrob(paste0("EE[Y|A=", a, ",W]"), gp=gpar(fontsize=18)))  
  
  if(!is.na(save_plot)){
    ggsave(save_plot, plot=p, width = 8, height = 5, dpi = 800)
  }
