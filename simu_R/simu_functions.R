...
run_simu_smoothness_adaptive_HAL_rep <- function(simu.num, eval_points, y_type, n, rounds){
  ...
  for(r in 1:rounds){
    ...
    while(!success){
      result <- tryCatch({
        result <- run_simu_smoothness_adaptive_HAL_1round(simu.num, eval_points, y_type, n=n)
        success <- TRUE
        result
      }, error = function(e) {
...
run_simu_smoothness_adaptive_HAL_1round <- function(simu.num, eval_points, y_type, n){
  ...
  Y <- as.numeric(as.matrix(dat %>% select(all_of(y_name))))
  X <- dat %>% 
    select(all_of(x_names)) %>% 
    mutate_if(sapply(., is.factor), as.numeric)
  
  
  dSL_fit <- fit_SL_smoothness_adaptive_HAL(dat, X, Y, x_names, y_name, y_type)
  ...
  names(results) = c("halcv0", "halcv0_sKnots", "halcv1", "halcv1_sKnots", 
